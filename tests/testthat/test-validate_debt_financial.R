test_that("validate_debt_financial() function exists", {
  expect_true(
    class(validate_debt_financial) == "function"
  )
})

test_that("validate_debt_financial() uses a function name that conforms to the list of data_object_names", {
  expect_true(
    sub("^validate_", "", deparse(substitute(validate_debt_financial))) %in% data_object_names
  )
})
