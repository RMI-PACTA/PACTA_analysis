...
Apparently, it's expected to come from the global environment (which is a bad idea); surprisingly, it is defined in an interactive session, but not in a non-interactive session:
...
I stop here. The rest fails until we successfully `source("web_tool_script_1.R")`.
