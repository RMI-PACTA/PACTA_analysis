...
Number of holdings and aggregated value in USD of the portfolio input, by investor and asset type

```{r}
total_overview_inv <- result_data_qa %>% 
  group_by(investor_name, asset_type) %>% 
  summarise(total_holdings = n(),
            sum_value_usd = round(sum(value_usd, na.rm = T))) %>% 
  ungroup()
datatable(total_overview_inv, caption = "Overall # of holdings and value of the input by Investor and asset type")
```

Number of holdings and aggregated value in USD of the portfolio input, by investor and flag type

```{r}
total_overview_inv_flag <- result_data_qa %>% 
  group_by(investor_name, flag) %>% 
  summarise(total_holdings = n(),
            sum_value_usd = round(sum(value_usd, na.rm = T))) %>% 
  ungroup()
# add share value investor
datatable(total_overview_inv_flag, caption = "Overall # of holdings and value of the input by Investor and flag")
```
...
investors_portfolio_overview <- result_data_qa %>% 
  ...
  summarise(total_holdings = n(),
            sum_value_usd = round(sum(value_usd, na.rm = T))) %>% 
  ungroup() %>% 
  mutate(value_weight = round(sum_value_usd / sum(sum_value_usd), 3))
...
currency_overview <- result_data_qa %>% 
  ...
  ungroup()
datatable(currency_overview, caption = "Aggregate market in original currencies grouped by missing currency information")
...
missing_currency <- result_data_qa %>% 
  ...
  ungroup() %>% 
  left_join(
    investors_portfolio_overview %>% select(investor_name, portfolio_name, sum_value_usd), by = c("investor_name", "portfolio_name")
    ) %>% 
  mutate(share_missing_value = round(
    value_usd_miss_currency/sum_value_usd, 4))
datatable(missing_currency, caption = "Missing aggregate market value due to lack of currency/exchange rate info across overall input by currency")
...
non_pacta_ALD_CB_detail <- result_data_qa %>% 
  ...
  summarise(sum_value_usd = round(sum(value_usd, na.rm=T))) %>% 
  ungroup() %>% 
  mutate(share_total_value = round(sum_value_usd/
                                     (total_overview %>%
                                        filter(asset_type=="Bonds") %>%
                                        pull(sum_value_usd)),6)) %>% 
  arrange(desc(share_total_value))
