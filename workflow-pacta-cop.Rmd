...
The peer comparison component of the interactive report requires you to generate the results for all participants offline first. This can be done by running the PACTA offline workflow. You do not need to generate reports, just the output files. It is important to prepare the portfolio files representing all peers correctly to do so. 
...
These should be saved in the folder pacta_data/[timestamp] with the above format. 
...
This is entered by the user on the platform. If you require different peer groups selection options get in contact with Wim, or an alternative combination of these for your results this can be programmed into create_interactive_report. 
...
# Saving the files into the correct locations. 
...
```{r echo = FALSE}
 ...
 render(file.path("pacta_data_qa.Rmd"),
...
# Here is a list of the peer groups being checked for. If your project contains different ones, enter them here. 
...
if (file.exists(file.path(data_path, paste0(project_code, "_peers_equity_results_portfolio.rda")))){
  peers_equity_results_portfolio <- read_rds(file.path(data_path, paste0(project_code, "_peers_equity_results_portfolio.rda")))
...
if(file.exists(file.path(data_path, paste0(project_code, "_peers_bonds_results_portfolio.rda")))){
  peers_bonds_results_portfolio <- read_rds(file.path(data_path, paste0(project_code, "_peers_bonds_results_portfolio.rda")))
...
if (file.exists(file.path(data_path, paste0(project_code, "_peers_equity_results_portfolio_ind.rda")))){
  peers_equity_results_user <- read_rds(file.path(data_path, paste0(project_code, "_peers_equity_results_portfolio_ind.rda")))
...
if(file.exists(file.path(data_path, paste0(project_code, "_peers_bonds_results_portfolio_ind.rda")))){
  peers_bonds_results_user <- read_rds(file.path(data_path, paste0(project_code, "_peers_bonds_results_portfolio_ind.rda")))
...
The executive summary will not print if the files do not exist
...
Feel free to repeast this process, changing the test_investor or test_portfolio. 
...
This project should be added to the docker image to ensure that it is tested for critical failure with every new release of the online tool.  
