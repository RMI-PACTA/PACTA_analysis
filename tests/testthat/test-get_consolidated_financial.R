test_that("get_consolidated_financial() function exists", {
  expect_true(
    class(get_consolidated_financial) == "function"
  )
})

test_that("get_consolidated_financial() uses a function name that conforms to the list of data_object_names", {
  expect_true(
    sub("^get_", "", deparse(substitute(get_consolidated_financial))) %in% data_object_names
  )
})

test_that("get_consolidated_financial() returns a data.frame", {
  skip_check_but_run_test()
  result <- get_consolidated_financial(here::here("inst/extdata"))
  expect_true(
    inherits(result, "data.frame")
  )
})

test_that("get_consolidated_financial() returns a data.frame with 19 columns", {
  skip_check_but_run_test()
  result <- get_consolidated_financial(here::here("inst/extdata"))
  expect_true(
    ncol(result) == 19
  )
})

test_that("get_consolidated_financial() returns a data.frame with the proper column names", {
  skip_check_but_run_test()
  result <- get_consolidated_financial(here::here("inst/extdata"))
  expect_equal(
    names(result),
    c("company_id", "bloomberg_id", "company_name", "country_of_domicile",
      "corporate_bond_ticker", "equity_ticker", "mapped_sector", "bics_sector",
      "bics_subgroup", "icb_subgroup", "bclass4", "has_asset_level_data",
      "has_assets_in_matched_sector", "sectors_with_assets", "market_cap",
      "free_float_shares", "current_shares_outstanding",
      "current_shares_outstanding_all_classes", "financial_timestamp")
  )
})

test_that("get_consolidated_financial() returns a data.frame with proper character class columns", {
  skip_check_but_run_test()
  result <- get_consolidated_financial(here::here("inst/extdata"))
  char_cols <- c("company_name", "country_of_domicile", "corporate_bond_ticker",
                 "equity_ticker", "mapped_sector", "bics_sector",
                 "bics_subgroup", "icb_subgroup", "bclass4",
                 "sectors_with_assets", "financial_timestamp")
  expect_true(
    all(sapply(result[char_cols], class) == "character")
  )
})

test_that("get_consolidated_financial() returns a data.frame with proper numeric class columns", {
  skip_check_but_run_test()
  result <- get_consolidated_financial(here::here("inst/extdata"))
  num_cols <- c("company_id", "bloomberg_id", "market_cap", "free_float_shares", "current_shares_outstanding", "current_shares_outstanding_all_classes")
  expect_true(
    all(sapply(result[num_cols], class) == "numeric")
  )
})

test_that("get_consolidated_financial() returns a data.frame with proper logical class columns", {
  skip_check_but_run_test()
  result <- get_consolidated_financial(here::here("inst/extdata"))
  log_cols <- c("has_asset_level_data", "has_assets_in_matched_sector")
  expect_true(
    all(sapply(result[log_cols], class) == "logical")
  )
})
