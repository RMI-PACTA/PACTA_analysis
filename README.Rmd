...
[Install Docker](https://docs.docker.com/get-docker/) then the entire computing
environment with:
...
For an example on how to use Docker, consider how I created the document you are
now reading:

* I first moved into the parent directory of PACTA_analysis/ and other siblings
of the PACTA family.
...
* Then I run (`docker run`) an ephemeral (`--rm`) interactive (`-ti`) container
from the image `2dii/r-packages`, and created a volume (`-v`) to access
everything in my local working directory (`$(pwd)`) from the `/home` directory
...
* Docker dropped me at the root directory of the container, so I moved into
PACTA_analysis/.
...
PACTA_analysis/, under the same parent directory. To make them easier to find,
all siblings include an empty file called ".pacta"
