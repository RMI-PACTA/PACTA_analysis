...
(files <- dir_ls(here::here("deduplicate"), regexp = pattern))
...
config_paths <- path_parent(sub("^../(.*)", "\\1", config_paths))
...
out_1 <- here("working_dir", "30_Processed_Inputs")

expect_false(dir_has_files(out_1))

# FIXME: Surprisingly, this line fails from rmd but runs on the console
source(here("web_tool_script_1.R"))
...
```{r message=FALSE}
out_2 <- here("working_dir", "40_Results")

expect_false(dir_has_files(out_2))
source(here("web_tool_script_2.R"))
...
```{r message=FALSE}
out_3 <- here("working_dir", "50_Outputs")

expect_false(dir_has_files(out_3))
source(here("web_tool_script_3.R"))
