...
# this is still very ugly, I will tidy it up asap
if (
  ...
  result_data_qa <- readRDS(paste0(project_location,"/30_Processed_Inputs/",project_name,"_total_portfolio.rda"))
  bonds_results_company <- readRDS(paste0(project_location,"/40_Results/Meta Investor/Bonds_results_company.rda"))
  equity_results_company <- readRDS(paste0(project_location,"/40_Results/Meta Investor/Equity_results_company.rda"))
} else if (
  c("Bonds_results_company.rda") %in% list.files(results_path) &
  c(paste0(project_name, "_total_portfolio.rda")) %in% list.files(paste0(proc_input_path, "/"))
) {
...
funds_coverage <- result_data_qa %>% 
  ...
  summarise(value_usd_by_fund = round(sum(value_usd, na.rm=T))) %>% 
  ungroup() %>% 
  mutate(
    overall_value_usd = (investors_overview %>%
                           filter(investor_name=="Meta Investor") %>%
                           pull(sum_value_usd_by_inv)),
    share_fund_value_total = round(value_usd_by_fund/overall_value_usd, 5)
    ) %>% 
...
funds_coverage_missing_value <- result_data_qa %>% 
  ...
  select(fund_isin, currency, value_usd_missing_by_fund, value_usd_by_fund, overall_value_usd) %>% 
  mutate(
    share_missing_value_by_fund = round(value_usd_missing_by_fund/value_usd_by_fund, 5),
