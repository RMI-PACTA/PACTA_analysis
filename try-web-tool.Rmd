...
#### 3.1. Ensure that there is a config file with name "TestPortfolio_Input_PortfolioParameters.yml" in working_dir/10_Parameter_File/ (this should already be the case, the content should not matter too much, as far as I can tell….)
...
If I continue working interactively, and craete the missing directory, the script web_tool_script_1.R seems to continue to the end but I get this warning:
...
I suspect the problem is the double "/" as in "...//40_Results/...", which might results from creating paths with `paste0()` instead of dedicated functions such as `fs::path()` or `here::here()`.
