...
define_benchmarks <- function(){
  ...
  if(!start_year %in% cb_market$year){stop(paste0("Check that start years align. CB Market Start year = ",min(cb_market$year),". Report start year = ", start_year))}
}
  ...
  if (PlotChart == TRUE){
    ...
    if (sector_to_plot == "Aviation") {
      PlotData <- Data %>%
        ...
        summarise(plan_emission_factor = mean(plan_sec_emissions_factor,na.rm=T) * 1000,
